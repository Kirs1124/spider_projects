...
if __name__ == '__main__':
    ...
    word_result.drop([17],inplace=True)
    print(word_result[:20])
    ...
    words = dict(tags)
    cloud = wordcloud.WordCloud(width=1920,height=1080,font_path="./simkai.ttf",background_color="white",mask=bear,max_words=500,max_font_size=150)
    word_cloud = cloud.generate_from_frequencies(words)
    plt.figure(figsize=(12,9))
    plt.imshow(word_cloud)
    # 词云保存
    plt.savefig('./鬼灭之刃词云.jpg',dpi = 2000)
    plt.show()
